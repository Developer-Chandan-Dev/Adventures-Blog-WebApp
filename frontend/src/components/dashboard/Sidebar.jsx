...
const Sidebar = () => {
  ...
  return (
    <div
      className={`w-[6.5vw] ${
        isSidebarOpen === true ? "lg:w-[6.5vw]" : "lg:w-[18vw]"
      } dashboard_sidebar h-full bg-white drop-shadow relative text-sm lg:text-base`}
    >
      <ul
        className={`w-[5vw] ${
          isSidebarOpen === true ? "lg:w-12 lg:mx-auto" : "lg:w-11/12"
        } mx-auto mt-5  `}
      >
        {navItems &&
          navItems.map(({ name, slug, svg }, index) => (
            <Link to={slug} key={index}>
              ...
              >
                ...
                <span
                  className={`hidden ${
                    isSidebarOpen === true ? "lg:hidden" : "lg:block"
                  } `}
                >
                  {name}
                </span>
      ...
      <button
        ...
        className="w-7 hidden lg:flex h-7 rounded-md bg-white drop-shadow absolute -right-[14px] bottom-10 justify-center items-center cursor-pointer transition-all hover:bg-slate-50"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 256 512"
